...
本次物流从前期验证到最终实现所有功能花了很多时间，吸取了之前的经验，也有新技术的应用，本文章把我所完成的智能物流的电控方向所用技术总结起来，方便以后随时阅读和查看。

运行视频：[物流小车_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1aeZmYMEMx/?spm_id_from=333.1007.0.0)
...
<div style="text-align: center;"><img src="文档\image\readme\物流机器人整体视图3.jpg" alt="整体视图"></div>
...
本次PCB最重要的设计思路就是堆叠式PCB设计，堆叠式PCB可以实现更快的**电路维修**，替换，实现电路的**模块化**，充分利用**垂直空间**。例如功率板和主控板的堆叠设计，充分利用了物流车的垂直空间，调试时主控板损坏，又或者主控板需要替换升级，我们只需要替换掉主控板就可以了，不需要维护功率板。截止到校赛，一共设计了四块PCB，分别是**功率板**、**主控板**、**云台顶部、底部分线板、**显示板、灯光板、泰山派拓展版等。功率板连接电池，负责整车电源控制，主控板使用**STM32F4**主控，引出多种通讯接口，满足各种外设控制需求。分线板通过40pin的FC排线连接主控板，然后引出各种接口，接电机、树莓派（泰山派）等设备。显示板使用74hc595串行芯片实现控制数码管显示任务码。具体文件可以参考[PCB\25工创PCB.eprj](./PCB/25工创PCB.eprj)文件，PCB文件使用嘉立创绘制。
...
<div style="text-align: center;"><img src="文档\image\readme\功率板底层layout.png" alt="堆叠设计"></div>
...
<div style="text-align: center;"><img src="文档\image\readme\功率板+主控板3d模型.png" alt="堆叠设计"></div>
...
<div style="text-align: center;"><img src="文档\image\readme\主控板顶层layout.png" alt="堆叠设计"></div>
...
<div style="text-align: center;"><img src="文档\image\readme\主控板和功率板堆叠设计.png" alt="堆叠设计"></div>
...
设计分线板的目的是为了**更简洁**的接线，还有把**陀螺仪**放在物流车的**机械中心**。分线板分为**两块**，一块是在云台下面，另外一个是云台上面，通过电滑环连接。分线板大部分是接口元件，还有一个AMS1117从5V降到3.3V供陀螺仪使用。由于分线板是放在机器内部，比较难拆出来，所以设计的时候就想着要电路要简洁可靠，基本没有什么元件，这样才可靠性高，可靠性高意味着不用经常拆出来维护，最终设计出来的效果也确实如此，上面只有两个元器件，一个是降压芯片还有一个陀螺仪，其他都是接口，连接电机、舵机等各种设备。平时容易出问题的PCB都放在机器外面比较好拆卸的地方，通过一个40pin的FC排线连接分线板，拆卸的时候只要拆出排线和几颗螺丝就能拆除主控板和功率板，这种设计更能提高维护效率。在设计分线板的原理图的时候要注意**FC排线线序**的问题，不然线序错了接上就烧了。

* 云台底部分线板原理图

<div style="text-align: center;"><img src="文档\image\readme\云台底部分线板V2原理图.png" alt="分线板"></div>
...
<div style="text-align: center;"><img src="文档\image\readme\云台底部分线板V2顶层layout.png" alt="堆叠设计"></div>

* 云台顶部分线板原理图
  云台顶部分线板预留了泰山派供电，泰山派通讯，云台电机通讯，舵机控制。

<div style="text-align: center;"><img src="文档\image\readme\云台顶部分线板原理图.png" alt="堆叠设计"></div>

* 云台顶部分线板layout

<div style="text-align: center;"><img src="文档\image\readme\云台顶部分线板顶层layout.png" alt="堆叠设计"></div>

顶部分线板在给视觉模块（泰山派）那路装了很多电容，全部加起来有1F，设计的目的是放置泰山派在电机启动时造成整车低电压导致泰山派重启。

#### 显示板

* 显示板原理图

<div style="text-align: center;"><img src="文档\image\readme\显示板原理图V2.png" alt="堆叠设计"></div>

* 显示板顶层layout

<div style="text-align: center;"><img src="文档\image\readme\显示板V2顶层layout.png" alt="堆叠设计"></div>

#### 灯光板

灯光板设计目的是为了给视觉补光，做了个环形光源。使用stc15w204s单片机，仅有8根引脚，十分适合来控制rgb灯珠。灯珠用的是ws2812。

* 灯光板原理图

<div style="text-align: center;"><img src="文档\image\readme\灯光板原理图.png" alt="堆叠设计"></div>

如下图所示环形光源

<div style="text-align: center;"><img src="文档\image\readme\灯光板顶层layout.png" alt="堆叠设计"></div>
...
2. ~~目前电路板还存在电机上电时**电压不稳**的情况，推测是电池的电流输出能力不够或者升降压芯片的动态响应不够好的原因，会在电机上电时造成其他路有个瞬间低电压，造成其他路设备重启，可以尝试**增大输出电容**来解决这个问题。~~（已解决，解决方案在顶部分线板与泰山派连接的5v电源升压到6v，另外添加1F电容作为低谷储能电容，实测5000uf电容就可以）
3. ~~主控板的双层typec接口目前会概率出现无法识别的情况，估计是识别电阻的问题，可以通过换用更熟悉的单层typec接口来解决或者是尝试更换识别电阻。~~（已解决，解决方案直接使用stm32f4串口连接视觉设备串口，不通过USB）
4. ~~为了显示板的布线考虑，原理图74HC595连接数码管的方式是乱序的，编写驱动程序的时候要注意。~~（74HC595芯片的显示板不使用，原因：电路板功能验证未通过，改用V2任务码显示板）
...
<div style="text-align: center;"><img src="文档\image\readme\物流车顶视图.png" alt="堆叠设计"></div>
