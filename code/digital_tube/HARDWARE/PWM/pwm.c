...
void TB6612_PWM_PIN_Init(u16 arr, u16 psc)
{
	...
	GPIO_InitStructure.GPIO_Pin = PWMA; //TIM2_CH4
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;  //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(PWMA_GPIO, &GPIO_InitStructure);
	GPIO_InitStructure.GPIO_Pin = PWMB; //TIM3_CH3
...
void pwm_out(u8 AO_or_BO, uint16_t duty)
{
	...
	if(AO_or_BO == AO) TIM_SetCompare4(TIM2, duty);
	else if(AO_or_BO == BO) TIM_SetCompare3(TIM3, duty);
	else return;
}
